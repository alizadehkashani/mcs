...
let buildgroupinformation = () => {
	...
	for(let i = 0; i < tracks["id"].length; i++){
		...
		for(let j = 0; j < 2; j++){
			...
			//cotnainer fro headline of next/currentgroup
...
let buildheader = () => {
	...
	let pathlogos = {
		0: "/mcs/dev/sponsors/knopf.jpg"
	};

	console.log(pathlogos);

	for(let i = 0; i < 8; i++){
		let logo = document.createElement("img");
		logo.setAttribute("src", pathlogos[0]);
